...
const App: React.FC = () => {
  ...
  const handleError = (msg: string) => {
      setError(msg);
      // Auto-clear error after 5 seconds if it's not a connection abort
  ...
  const handleVoiceChange = async (newVoice: string) => {
      ...
      if (connectionState === ConnectionState.CONNECTED) {
          addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: `Reconfiguring vocal synthesis to ${newVoice.toUpperCase()}...`, type: 'warning' });
          await disconnectFromGemini();
          await connectToGemini(newVoice);
      } else {
          addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: `Vocal identity set to ${newVoice.toUpperCase()}`, type: 'success' });
      }
  };
  ...
  const handleShutdown = async () => {
      addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: 'Shutdown sequence initiated...', type: 'warning' });
      await disconnectFromGemini();
  ...
  const handleReboot = async () => {
      addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: 'Reboot command received. Cycling power...', type: 'warning' });
      await disconnectFromGemini();
  ...
  const handleSystemAction = async (action: string, args: any): Promise<any> => {
    switch (action) {
      case 'scanSystem':
        addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: 'Initiating full system diagnostic scan...', type: 'info' });
        await new Promise(r => setTimeout(r, 800));
        addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: 'Scanning File System... OK', type: 'info' });
        await new Promise(r => setTimeout(r, 600));
        addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: 'Checking Network Protocols... OK', type: 'info' });
      ...
      case 'checkIntegrity':
        addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: 'Running system integrity verification...', type: 'info' });
        ...
        addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: `Integrity verification complete: ${newIntegrity}%`, type: 'success' });
      ...
      case 'openWebsite':
        ...
        addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: `Opening external protocol: ${site}`, type: 'success' });
        window.open(url, '_blank');
        return { success: true, message: `Opened ${site}` };

      case 'openFile':
        const fileName = args.fileName;
        addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: `Searching file system for: ${fileName}`, type: 'info' });
        await new Promise(r => setTimeout(r, 1000));
        addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: `File located. Executing ${fileName}...`, type: 'success' });
      ...
      case 'manageSystemPower':
        ...
        if (pAction === 'ON') {
             ...
             addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: 'Power check: Systems are already online.', type: 'info' });
      ...
      case 'adjustVolume':
        ...
        addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: `Adjusting master volume to ${vol}%`, type: 'success' });
      ...
      case 'changeVoice':
        ...
        if (validVoices.includes(newVoice)) {
            setVoiceName(newVoice);
            addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: `Reinitializing audio synthesis protocol: ${newVoice}`, type: 'warning' });
            
            // Reconnect logic with delay to allow tool response to 'conceptually' finish
            setTimeout(async () => {
                await disconnectFromGemini();
                await connectToGemini(newVoice);
            }, 500);
  ...
  const executeTextCommand = async (e: React.FormEvent) => {
    ...
    addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'USER', message: command, type: 'info' });
        ...
        if (candidates && candidates[0]) {
             ...
             for (const part of parts) {
                 if (part.functionCall) {
                     ...
                     addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'JARVIS', message: `Executing ${fc.name}...`, type: 'info' });
                     const result = await handleSystemAction(fc.name, fc.args);
                     // Note: In a real multi-turn chat we'd send the result back, but for single command this is sufficient visual feedback
                 }
                 if (part.text) {
                     addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'JARVIS', message: part.text, type: 'success' });
                 }
             }

             if (!toolCalled && !parts.some(p => p.text)) {
                 // Fallback if empty response
                 addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'JARVIS', message: "Processing complete.", type: 'info' });
             }
        }
    } catch (err: any) {
        const msg = err.message || "Unknown error occurred";
        addLog({ id: crypto.randomUUID(), timestamp: new Date().toLocaleTimeString(), source: 'SYSTEM', message: `Command failed: ${msg}`, type: 'error' });
  ...
  const getThemeBorder = () => {
     if (theme === 'red') return 'border-red-500 focus:border-red-400';
     if (theme === 'gold') return 'border-yellow-500 focus:border-yellow-400';
     return 'border-cyan-500 focus:border-cyan-400';
  };
  
  const getThemeText = () => {
     if (theme === 'red') return 'text-red-500 placeholder-red-800';
  ...
  const getInputClass = () => {
      ...
      classes += getThemeText() + " ";
      classes += getThemeBorder() + " ";
      
      // Add pulsing effect if idle
      if (!processingCommand && textCommand === '') {
          if (theme === 'red') classes += 'input-pulse-red ';
          else if (theme === 'gold') classes += 'input-pulse-gold ';
          else classes += 'input-pulse-cyan ';
      } else {
          // Steady glow if active
          if (theme === 'red') classes += 'shadow-[0_0_20px_rgba(239,68,68,0.4)] ';
          else if (theme === 'gold') classes += 'shadow-[0_0_20px_rgba(234,179,8,0.4)] ';
          else classes += 'shadow-[0_0_20px_rgba(6,182,212,0.4)] ';
      }
    ...
    <div className="relative w-screen h-screen bg-[#050505] text-white overflow-hidden flex flex-col md:flex-row">
      ...
      <div className={`z-10 w-full md:w-1/3 h-[45%] md:h-full p-4 md:p-8 order-2 md:order-1 flex flex-col transition-opacity duration-1000 ${powerState === 'ONLINE' ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="mt-4 relative">
             <form onSubmit={executeTextCommand} className="relative">
                <div className={`absolute -top-3 left-4 bg-black px-2 ${getThemeText()} text-xs font-bold tracking-widest`}>MANUAL OVERRIDE</div>
                <input 
                    type="text" 
                    value={textCommand}
                ...
                <button 
                    type="submit" 
                    className={`absolute right-2 top-1/2 -translate-y-1/2 ${getThemeText()} opacity-50 hover:opacity-100 uppercase text-xs font-bold tracking-wider`}
                    disabled={processingCommand}
                >
                    EXECUTE
                </button>
      ...
      <div className={`z-10 w-full md:w-1/3 h-[55%] md:h-full flex flex-col items-center justify-center relative order-1 md:order-2 transition-all duration-1000 ${powerState === 'ONLINE' ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
         ...
         <div className="absolute top-4 md:top-10 font-tech text-center opacity-80">
            <h1 className={`text-4xl font-black tracking-[0.2em] ${getThemeColor().split(' ')[0]}`}>J.A.R.V.I.S.</h1>
      ...
      <div className={`z-10 hidden md:flex w-1/3 h-full p-8 flex-col justify-center items-end order-3 transition-opacity duration-1000 ${powerState === 'ONLINE' ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="flex gap-2 flex-wrap justify-end max-w-[200px]">
               {['Puck', 'Zephyr', 'Kore', 'Fenrir', 'Charon'].map(v => (
                   <button 
                     ...
                     className={`px-2 py-1 text-[10px] font-mono border ${voiceName === v ? getThemeBorder() + ' ' + getThemeText() + ' bg-white/10' : 'border-gray-800 text-gray-600 hover:border-gray-500'} transition-all uppercase`}
                   >
                     {v}
                   </button>
               ))}
         ...
         <div className={`w-64 h-96 border-r-2 ${theme === 'red' ? 'border-red-500/30' : theme === 'gold' ? 'border-yellow-500/30' : 'border-cyan-500/30'} flex flex-col gap-2 items-end pr-4 opacity-60`}>
            {Array.from({length: 12}).map((_, i) => (
               <div key={i} className={`h-1 ${theme === 'red' ? 'bg-red-500' : theme === 'gold' ? 'bg-yellow-500' : 'bg-cyan-500'}`} style={{ width: `${Math.random() * 100}%`, opacity: Math.random() }}></div>
            ))}
            <div className="flex-1"></div>
            <div className={`text-right font-tech text-4xl ${theme === 'red' ? 'text-red-500' : theme === 'gold' ? 'text-yellow-500' : 'text-cyan-500'} opacity-50`}>MK-{Math.floor(Math.random() * 85)}</div>
         </div>
      </div>
